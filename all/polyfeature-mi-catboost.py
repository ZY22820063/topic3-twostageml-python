...
for i in range(1, round):
    ...
    # ===== 保留：生成多项式特征（以二次多项式为例，可自行调整 degree） =====
    ...
    # ===== 新增：先计算一次互信息，后续在 for 循环里直接使用 =====
    # mutual_info_regression 返回的是与 X_train_poly 各列对应的互信息评分
    mi_scores = mutual_info_regression(X_train_poly, y_train, discrete_features=False)
    # 对特征进行从高到低排序
    sorted_indices = np.argsort(mi_scores)[::-1]  # 互信息值从大到小排序
    # 排序后的特征名
    sorted_features = X_train_poly.columns[sorted_indices]
...
# 将结果保存为 CSV 文件
all_results_df.to_csv('poly_micatboost_results.csv', index=False)
all_feature_subsets_df.to_csv('poly_micatboost_feature_subsets.csv', index=False)
