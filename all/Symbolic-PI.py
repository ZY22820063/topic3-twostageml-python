...
for i in range(1, 11):
    ...
    # 使用整个训练集重新训练模型
    symbol_regressor.fit(X_train, y_train)
    best_formula = symbol_regressor._program  # 获取最佳公式
    
    # 在测试集上评估
    y_pred_test = symbol_regressor.predict(X_test)
...
# 保存所有结果
results_df.to_csv("SymbolicRegressor-nothick.csv", index=False)
print("All results have been saved to 'symbolic_regression_results.csv'.")
