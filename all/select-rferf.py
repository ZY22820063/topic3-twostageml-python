# -*- coding: utf-8 -*-
"""
Created on Thu Mar 28 15:43:42 2024
...
for i in range(1, 11):
    ...
    # 特征数量范围
    feature_range = range(7, 0, -1)  # 这里仅举例：从4减少到1，可根据需求改动

    # 存储当前数据集的结果
    results = []

    for n_features in feature_range:
        print(f"Dataset {i} | n_features: {n_features} | start")
        # RFE进行特征选择
        ...
        cv_rmse = -cross_val_score(
            rf_model, X_train[selected_features], y_train, 
            scoring='neg_root_mean_squared_error', cv=loo
        ).mean()
        ...
        print(f"Dataset {i} | n_features: {n_features} | end")
...
# 可以选择将结果保存为CSV文件
all_results_df.to_csv('ts-rferf.csv', index=False)
