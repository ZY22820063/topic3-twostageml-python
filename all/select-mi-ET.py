...
from sklearn.ensemble import ExtraTreesRegressor  # 使用 ExtraTreesRegressor
...
for i in range(1, 11):
    ...
    # 使用所有特征训练初始ExtraTrees模型
    et_model_full = ExtraTreesRegressor(random_state=42, n_jobs=-1)
    et_model_full.fit(X_train, y_train)
    
    # 计算排列重要性得分
    result = permutation_importance(et_model_full, X_train, y_train,
    ...
    # 计算综合特征重要性得分
    combined_scores = mi_scores_norm + pi_scores_norm  # 根据需要可调整权重
    combined_scores = combined_scores.sort_values(ascending=False)
    
    # 特征数量范围（此处仅做示例，可以根据实际需求调整）
    feature_range = range(7, 0, -1)  # 从4个特征到1个特征的示例
    ...
    for n_features in feature_range:
        ...
        # 基于选择的特征重新训练ExtraTrees模型
        et_model = ExtraTreesRegressor(random_state=42, n_jobs=-1)
        et_model.fit(X_train[selected_features], y_train)
    
        # 在测试集上评估模型
        y_pred_test = et_model.predict(X_test[selected_features])
        test_rmse = np.sqrt(mean_squared_error(y_test, y_pred_test))
        test_r2 = r2_score(y_test, y_pred_test)
    
        # 在训练集上评估模型
        y_pred_train = et_model.predict(X_train[selected_features])
        train_rmse = np.sqrt(mean_squared_error(y_train, y_pred_train))
        train_r2 = r2_score(y_train, y_pred_train)
    
        # Leave-One-Out 交叉验证
        loo = LeaveOneOut()
        cv_rmse_scores = -cross_val_score(et_model,
                                          X_train[selected_features],
                                          y_train,
                                          scoring='neg_root_mean_squared_error',
                                          cv=loo,
                                          n_jobs=-1)
        cv_r2_scores = cross_val_score(et_model,
                                       X_train[selected_features],
                                       y_train,
                                       scoring='r2',
                                       cv=loo,
...
# 将结果保存为 CSV 文件
all_results_df.to_csv('mi_et_results.csv', index=False)
all_feature_subsets_df.to_csv('mi_et_feature_subsets.csv', index=False)
