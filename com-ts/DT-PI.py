# -*- coding: utf-8 -*-
"""
Created on Wed Dec  4 14:48:56 2024
...
for i in range(1, 11):
    ...
    for train_index, val_index in kf.split(X_train):
        ...
        # 拟合模型
        tree_regressor.fit(X_train_cv, y_train_cv)
        
        # 预测并计算指标
        y_pred_train_cv = tree_regressor.predict(X_train_cv)
        y_pred_val_cv = tree_regressor.predict(X_val_cv)
    ...
    # 导出决策树规则（作为公式）
    tree_formula = export_text(tree_regressor, feature_names=selected_features)
...
print("Best Formula:")
print(best_group['formula'])

# 保存所有结果
results_df.to_csv("decision_tree_results.csv", index=False)
print("All results have been saved to 'decision_tree_results.csv'.")
