...
for i in range(1, round):
    ...
    # =========================
    # 使用 ExtraTrees 替代 CatBoost
    # =========================
    # 初始化ExtraTrees回归器，设置random_state以保证结果可重复
    ...
    for n_features in feature_range:
        ...
        extratrees_model = ExtraTreesRegressor(random_state=42, n_jobs=-1)
        extratrees_model.fit(X_train_poly[selected_features], y_train)
    
        # 在训练集上评估模型
        y_pred_train = extratrees_model.predict(X_train_poly[selected_features])
        train_rmse = np.sqrt(mean_squared_error(y_train, y_pred_train))
        train_r2 = r2_score(y_train, y_pred_train)
    
        # 十折交叉验证
        kf = KFold(n_splits=10, shuffle=True, random_state=42)
        cv_rmse_scores = -cross_val_score(extratrees_model, 
                                          X_train_poly[selected_features], 
                                          y_train, 
                                          scoring='neg_root_mean_squared_error', 
                                          cv=kf, 
                                          n_jobs=-1)
        cv_r2_scores = cross_val_score(extratrees_model, 
                                       X_train_poly[selected_features], 
                                       y_train, 
                                       scoring='r2', 
                                       cv=kf, 
        ...
        # 在测试集上评估模型
        y_pred_test = extratrees_model.predict(X_test_poly[selected_features])
...
# 将结果保存为 CSV 文件
all_results_df.to_csv('poly_rfeet.csv', index=False)
all_feature_subsets_df.to_csv('poly_rfeet_feature.csv', index=False)
